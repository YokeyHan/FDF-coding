...
from models.layers1 import  SpatialHeteroModel, sinkhorn,SSL_cluster,sim_global,aug_topology,aug_traffic,TemporalConvLayer,Pooler
...
class ST_decomposition(nn.Module):
    def __init__(
        ...
        self.pool=Pooler(input_len-2,32)
        self.SC=SSL_cluster(in_features=1, out_features=channels, t_len=input_len, dropout=0.5,\
    ...
    def forward(self, x):
        ...
        x0=x.unsqueeze(-1).permute(0, 3, 2, 1)
        x_1 = self.tconv11(x0)    # nclv
        ...
        x_aug=aug_traffic(t_sim_mx,x0, percent=0.001)
        
        s_loss,A,z1= self.SC(x0,x_aug,self.adj,adj1_aug)
        ...
        # 5) 计算 soft_centers
        centers = self.SC.shm.prototypes.weight.clone() #[L*C,K]

        # 6) 计算 residual 并 reshape 回 [B, L, N, C]
        distances = torch.cdist(z1, centers)  ## 输出形状: [B*N, K]
        cluster_ids = torch.argmin(distances, dim=1)    ## 形状: [B*N]
        # centers = centers.T
        res = (z1 - centers[cluster_ids]).view(B, N, L, self.channels).permute(0, 2, 1, 3)   #[B, L, N, C]

        
        if self.training and self.device.type == 'cuda':
            self.visualize_clusters(z1, centers, cluster_ids, B, N, L)

        return res, centers,  z1, s_loss
    ...
    def visualize_clusters(self, Z, centers, cluster_ids, B, N, L):
        # 将特征和聚类中心拼接在一起
