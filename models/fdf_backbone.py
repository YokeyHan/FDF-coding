...
class ST_decomposition(nn.Module):
    def __init__(
        ...
        # 原型头（空间聚类）
        self.cluster_model = SpatialHeteroModel(
            nmb_prototype=num_clusters,
    ...
    def forward(self, x):
        ...
        # 2) 子空间投影 & 归一化
        feats = s_feat.permute(0, 2, 1, 3).reshape(B * N, -1)   ## [B*N, L*C]
        self.cluster_model.init_prototypes(feats.shape[1])

        Z = F.normalize(feats, p=2, dim=1)                          # [M, C]

        # 3) Prototype logits + L2 归一化原型权重
        with torch.no_grad():
            w = self.cluster_model.prototypes.weight.data.clone()
            w = F.normalize(w, p=2, dim=1)
            self.cluster_model.prototypes.weight.copy_(w)
        logits = self.cluster_model.prototypes(Z)                   # [M, K]

        # 4) Sinkhorn-Knopp 得到软分配 q
        q = sinkhorn(logits.detach(),
                     epsilon=self.cluster_model.tau,
                     sinkhorn_iterations=3)                    #[B*N,K]         # [M, K]

        # 5) 计算 soft_centers
        centers = self.cluster_model.prototypes.weight.t().clone()  #[L*C,K]

        # 6) 计算 residual 并 reshape 回 [B, L, N, C]
        distances = torch.cdist(Z, centers.T)  ## 输出形状: [B*N, K]
        cluster_ids = torch.argmin(distances, dim=1)    ## 形状: [B*N]
        centers = centers.T
        res = (Z - centers[cluster_ids]).view(B, N, L, self.channels).permute(0, 2, 1, 3)   #[B, L, N, C]

        # 7) 子空间特征 z（这里直接用 residual）
        z = res  # [B, L, N, C]
        ...
        if self.training:
            ...
            diff = F.relu(self.sep_margin - dmat)                 # 只有 < margin 的才产生 loss
            # 只取上三角 i<j
            sep_loss = diff.triu(1).sum()
        else:
            sep_loss = torch.tensor(0., device=x.device)
                # —— 对比学习损失 ——
        contrastive_loss = self.contrastive_loss(Z, q, centers)

                # —— 可视化聚类结果 ——
        if self.training and self.device.type == 'cuda':
            self.visualize_clusters(Z, q, centers, cluster_ids, B, N, L)

        return res, centers, q, z, sep_loss, contrastive_loss
