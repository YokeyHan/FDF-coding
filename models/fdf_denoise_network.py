...
class mySTDecoderBlock(nn.Module):
    def __init__(self,
        ...
        if use_spatial:
            ...
            self.spatial_attn = FeatureLearning0(channels=channels, nheads=nheads, is_cross=is_cross_s)#FeatureLearning(channels=channels, nheads=nheads, target_dim=36,
                                 #              order=2, include_self=True, device=device, is_adp=True,
                                  #             adj_file=adj, proj_t=64,is_cross=is_cross_s)

        if use_temporal:
            self.temporal_attn = TemporalLearning(channels=channels, nheads=nheads, is_cross=is_cross_t)
...
class STDecoderBlock(nn.Module):
    ...
    def forward(self, x, timestep, mask=None, label_emb=None):
        ...
        # 时空注意力处理
        if self.use_spatial:
            x_spatial = self.spatial_attn(x_norm)
        else:
            x_spatial = x_norm

        if self.use_temporal:
            x_temporal = self.temporal_attn(x_spatial)
        else:
            x_temporal = x_spatial

        # 应用原始的LINEAR层
        # 重塑为 [B, T, N, emb_dim] 以便处理
        x_linear = x_temporal.permute(0, 2, 1, 3).reshape(B, T, N, -1)
...
class DecoderBlock(nn.Module):
    def __init__(self,
        ...
        self.LayerNorm = nn.LayerNorm(emb_dim)
        #self.LINEAR = LINEAR(n_channel, n_channel)
        self.LINEAR = ChebyKANLayer(n_channel, n_channel)
...
class Decoder(nn.Module):
    def __init__(
    ...
    ): #Decoder(self.pred_length , n_feat, self.adj, emb_dim, n_layer, dropout)
      super().__init__()
      self.adj = adj
      self.n_feat = n_feat
      self.blocks = nn.Sequential(*[
            nn.Sequential(
                ...
                nn.Dropout(0.1)  # 每个block后添加Dropout
...
class fdf_denoise_network(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        super().__init__()
        #self.emb = myConv_MLP(n_feat, emb_dim, dropout=dropout)
        self.adj = adj
        self.z_dim = z_dim  # 计算每个节点的特征维度
        self.N = n_feat // z_dim
        #self, in_dim, emb_dim, N, dropout
        self.emb = myConv_MLP(z_dim, emb_dim, self.N, dropout)  # in_dim设为z_dim
    ...
    def forward(self, input, t, cond, padding_masks=None):
        ...
        batch_size, L, feature_dim = cond.shape     #[B,L,N*z_dim]
        input = input.reshape(batch_size,L,self.N, -1)  #[B,L,N,z_dim]
        num_patches = self.seq_length // self.patch_len   #96/8 = 12
        cond_patches = cond[:, :num_patches * self.patch_len, :].view(batch_size, num_patches, self.patch_len, feature_dim)  #[B,L,N*z_dim]-->[B,num_patch,patch_len,N*z_dim]
        ...
        emb = self.emb(input)    # Conv_MLP: [B, seq_len, n_feat] -> [B, seq_len, emb_dim]  [B,L,N*z_dim]-> [B, seq_len, emb_dim]
        inp_dec = self.pos_dec(emb)   # LearnablePositionalEncoding: 维度不变 [B, seq_len, emb_dim]
        output = self.decoder(inp_dec, t, padding_masks=padding_masks)    #[4,96,207,4] [B,L,N,z_dim]  # [B, seq_len, emb_dim]
        output = output.reshape(batch_size, self.pred_length, -1)

        #output = self.feat_linear(output)  #.reshape(batch_size, self.pred_length,-1))
                  #.reshape(batch_size, L,-1)     # [B, seq_len, n_feat]
        # 确保feature_dim和N*emb_dim相同
